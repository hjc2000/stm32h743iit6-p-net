...
static void pf_put_str(
	...
	uint16_t *p_pos)
{
	uint16_t str_len = (uint16_t)strlen(reinterpret_cast<char const *>(p_src));
	uint16_t ix;
...
static void pf_put_rpc_uuid(
	bool is_big_endian,
	pf_rpc_uuid_type_t const *p_uuid,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_time_timestamp(
	bool is_big_endian,
	pf_log_book_ts_t const *p_time_ts,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
static void pf_put_submodule_diff(
	bool is_big_endian,
	pf_submodule_diff_t const *p_diff,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_control(
	...
	pf_control_block_t const *p_res,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_pnet_status(
	bool is_big_endian,
	pnet_pnio_status_t const *p_status,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_read_result(
	bool is_big_endian,
	pf_iod_read_result_t const *p_res,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos,
...
void pf_put_write_result(
	bool is_big_endian,
	pf_iod_write_result_t const *p_res,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_log_book_data(
	bool is_big_endian,
	pf_log_book_t const *p_log_book,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
static void pf_put_diag_device(
	...
	pf_dev_filter_level_t filter_level,
	pf_diag_filter_level_t diag_filter,
	pf_ar_t const *p_ar, /* If != NULL only include those belonging to p_ar */
	pf_device_t const *p_device,
	uint32_t api_id,
	uint16_t slot_nbr,
	uint16_t subslot_nbr,
...
void pf_put_diag_data(
	...
	pf_dev_filter_level_t filter_level,
	pf_diag_filter_level_t diag_filter,
	pf_ar_t const *p_ar, /* If p_ar != NULL only include those belonging to p_ar
						  */
	uint32_t api_id,
	uint16_t slot_nbr,
	uint16_t subslot_nbr,
...
void pf_put_alarm_fixed(
	bool is_big_endian,
	pf_alarm_fixed_t const *p_alarm_fixed,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_alarm_block(
	bool is_big_endian,
	pf_block_type_values_t bh_type,
	pf_alarm_data_t const *p_alarm_data,
	uint32_t maint_status,
	uint16_t payload_usi, /* pf_usi_values_t */
	uint16_t payload_len,
	uint8_t const *p_payload, /* Union of may types - see below */
	pnet_pnio_status_t const *p_status,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
...
void pf_put_substitute_data(
	...
	uint16_t sub_mode,
	uint8_t iocs_len,
	uint8_t const *p_iocs,
	uint8_t iops_len,
	uint8_t const *p_iops,
	uint16_t data_len,
...
void pf_put_pdport_data_check(
	...
	uint16_t slot,
	uint16_t subslot,
	pf_check_peer_t const *check_peer,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
	...
	uint16_t *p_pos)
{
	...
	pf_lldp_peer_info_t const *p_peer_info = &p_port_data->lldp.peer_info;
...
void pf_put_pdport_statistics(
	bool is_big_endian,
	pnal_port_stats_t const *p_port_stats,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_pdinterface_data_real(
	bool is_big_endian,
	pnet_ethaddr_t const *mac_address,
...
void pf_put_peer_to_peer_boundary(
	bool is_big_endian,
	pf_adjust_peer_to_peer_boundary_t const *p_peer_to_peer_boundary,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
static void pf_put_rpc_floor_1_uuid(
	bool is_big_endian,
	pf_floor_uuid_t const *p_floor,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
	uint16_t lhs_length = (uint16_t)(sizeof(p_floor->protocol_id) + sizeof(p_floor->uuid) +
...
static void pf_put_rpc_handle(
	bool is_big_endian,
	pf_rpc_handle_t const *p_handle,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
...
void pf_put_lookup_response_data(
	pnet_t *net,
	bool is_big_endian,
	pf_rpc_lookup_rsp_t const *p_lookup_rsp,
	uint16_t res_len,
	uint8_t *p_bytes,
	uint16_t *p_pos)
{
