 ...
 */
static char const *pf_pdport_get_filename(int loc_port_num)
{
...
bool pf_pdport_is_a_fast_port_in_use(pnet_t *net)
{
	...
	pnet_port_cfg_t const *port_config;
#endif
...
void pf_pdport_remove_data_files(char const *file_directory)
{
	int port;
...
void pf_pdport_ar_connect_ind(pnet_t *net, pf_ar_t const *p_ar)
{
	(void)p_ar;
	int port;
	pf_port_iterator_t port_iterator;
...
int pf_pdport_read_ind(
	pnet_t *net,
	pf_ar_t *p_ar,
	pf_iod_read_request_t const *p_read_req,
	pf_iod_read_result_t const *p_read_res,
	uint16_t res_size,
	uint8_t *p_res,
	uint16_t *p_pos)
{
...
static int pf_pdport_write_data_check(
	pnet_t *net,
	pf_ar_t const *p_ar,
	pf_iod_write_request_t const *p_write_req,
	int loc_port_num,
	...
	pnet_result_t *p_result)
{
	...
	pf_port_data_check_t port_data_check{};
	...
	pnet_result_t *p_result)
{
	...
	uint16_t pos = 0;
	pf_get_info_t get_info;
	pf_port_data_adjust_t port_data_adjust{};
...
pnal_eth_status_t pf_pdport_get_eth_status_filtered_mau(
	...
	int loc_port_num)
{
	pnet_port_cfg_t const *p_port_config =
	...
	eth_status.operational_mau_type = pf_pdport_filter_mau_type(
		eth_status.operational_mau_type,
		eth_status.running,
		static_cast<pnal_eth_mau_t>(p_port_config->default_mau_type));
