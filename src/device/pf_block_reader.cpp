	...
	pf_exp_submodule_t *p_sub)
{
	...
	LOG_DEBUG(PNET_LOG,
			  ...
			  pf_cmdev_submod_dir_to_string(static_cast<pnet_submodule_dir_t>(p_sub->submodule_properties.type)));
}
	...
	pf_lldp_name_of_device_mode_t *name_of_device_mode)
{
	...
	temp = pf_get_uint32(p_info, p_pos);
	*name_of_device_mode = static_cast<pf_lldp_name_of_device_mode_t>(pf_get_bits(temp, 0, 1));
}
