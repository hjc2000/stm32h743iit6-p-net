...
void pf_fspm_im_show(pnet_t const *net)
{
...
void pf_fspm_logbook_show(pnet_t const *net)
{
	uint16_t ix;
...
void pf_fspm_option_show(pnet_t const *net)
{
 ...
 */
int pf_fspm_validate_configuration(pnet_cfg_t const *p_cfg)
{
	uint16_t const im_mask =
...
{
	pf_im_nvm_t file_im;
	char const *p_file_directory = pf_cmina_get_file_directory(net);
 ...
 */
static void pf_fspm_set_im_location(pnet_t *net, char const *location)
{
	os_mutex_lock(net->fspm_im_mutex);
	snprintf(
		net->fspm_cfg.im_1_data.im_tag_location,
...
void pf_fspm_save_im_location(pnet_t *net, char const *location)
{
	pf_fspm_set_im_location(net, location);
	(void)pf_bg_worker_start_job(net, PF_BGJOB_SAVE_IM_NVM_DATA);
}

int pf_fspm_init(pnet_t *net, pnet_cfg_t const *p_cfg)
{
...
{
	os_mutex_lock(net->fspm_im_mutex);
	memset(net->fspm_cfg.im_1_data.im_tag_function,
		   ' ',
	...
	memset(net->fspm_cfg.im_4_data.im_signature,
		   0,
...
void pf_fspm_get_default_cfg(pnet_t *net, pnet_cfg_t const **pp_cfg)
{
	if (pp_cfg != NULL)
	{
...
int16_t pf_fspm_get_min_device_interval(pnet_t const *net)
{
...
int pf_fspm_cm_read_ind(
	pnet_t *net,
	pf_ar_t const *p_ar,
	pf_iod_read_request_t const *p_read_request,
...
int pf_fspm_cm_write_ind(
	pnet_t *net,
	pf_ar_t const *p_ar,
	pf_iod_write_request_t const *p_write_request,
...
void pf_fspm_alpmr_alarm_ack_cnf(pnet_t *net, pf_ar_t const *p_ar, int res)
{
