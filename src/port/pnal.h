...
#define PNAL_IPADDR_BROADCAST ((uint32_t)0xffffffffUL)
	 ...
	 *          -1 if an error occurred.
	 */
	int pnal_get_port_statistics(char const *interface_name, pnal_port_stats_t *port_stats);
	 ...
	 *         -1 if an error occurred.
	 */
	int pnal_snmp_init(pnet_t *net, pnal_cfg_t const *pnal_cfg);
	 ...
	 */
	int pnal_get_macaddress(char const *interface_name, pnal_ethaddr_t *p_mac);
