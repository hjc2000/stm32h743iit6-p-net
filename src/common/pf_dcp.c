 ...
 */
static pnet_ethaddr_t const dcp_mc_addr_hello = {
...
static pnet_ethaddr_t const mac_nil = PF_MAC_NIL;
...
static pf_dcp_opt_sub_t const device_options[] = {
 ...
 */
static void pf_dcp_restart_sam_timeout(pnet_t *net, pnet_ethaddr_t const *mac)
{
 ...
 *         false if not.
 */
static bool pf_dcp_check_sam(pnet_t *net, pnet_ethaddr_t const *mac)
{
...
bool pf_dcp_check_destination_address(
    pnet_ethaddr_t const *local_mac,
    pnet_ethaddr_t const *destination_mac)
{
    return (((destination_mac->addr[0] & 1) == 0) && /* Not multi-cast */
 ...
 * @return
 */
static int pf_dcp_put_block(uint8_t *p_dst,
                            uint16_t *p_dst_pos,
                            uint16_t dst_max,
                            uint8_t opt,
                            uint8_t sub,
 ...
 * @return
 */
static int pf_dcp_get_req(pnet_t *net,
                          uint8_t *p_dst,
                          uint16_t *p_dst_pos,
                          uint16_t dst_max,
                          uint8_t opt,
    ...
    void *p_arg) /* Not used */
{
    ...
    // andre test code
    // int i;
    //  uint8_t *temp = (uint8_t *)p_buf->payload;
    // printf("p_buf = ");
    // for (i=0; i<80; i++)
    // {
    //    printf("%02x ", temp[i]);
    ...
    *p_dst_dcphdr = *p_src_dcphdr;
    ...
    if ((ret == 0) && (match == true))
    {
        ...
        // printf("\r\n");
#if 0
      pf_eth_send_on_management_port (net, p_rsp);
#else
        (void)pf_scheduler_add(
            net,
            response_delay,
...
void pf_dcp_init(pnet_t *net)
{
    ...
    printf("sizeof (pf_dcp_header_t) is %d\r\n", sizeof(pf_dcp_header_t));
    printf("sizeof (pf_dcp_block_hdr_t) is %d\r\n", sizeof(pf_dcp_block_hdr_t));
