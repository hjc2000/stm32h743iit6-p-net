	...
	uint32_t crep)
{
	...
	{
		pnal_buf_free(reinterpret_cast<pnal_buf_t *>(p_cpm->p_buffer_cpm));
	}
	if (p_cpm->p_buffer_app != NULL)
	{
		pnal_buf_free(reinterpret_cast<pnal_buf_t *>(p_cpm->p_buffer_app));
	}
...
static void pf_cpm_put_buf(pnet_t *net, pf_cpm_t *p_cpm, pnal_buf_t **pp_buf)
{
	...
	*pp_buf = reinterpret_cast<pnal_buf_t *>(p);
	p_cpm->new_buf = true;
	...
	void *p_arg)
{
	...
	pf_iocr_t *p_iocr = reinterpret_cast<pf_iocr_t *>(p_arg);
...
static int pf_cpm_driver_sw_get_data_status(pf_cpm_t const *p_cpm, uint8_t *p_data_status)
{
	*p_data_status = p_cpm->data_status;
	return 0;
}

static void pf_cpm_driver_sw_show(pnet_t const *net, pf_cpm_t const *p_cpm)
{
...
{
	static pf_cpm_driver_t const drv = {
