...
static char const org_id_pnio[] = {0x00, 0x0e, 0xcf};
static char const org_id_ieee_8023[] = {0x00, 0x12, 0x0f};
...
static pnet_ethaddr_t const lldp_dst_addr = {
...
static void pf_lldp_add_chassis_id_tlv(
	pf_lldp_chassis_id_t const *p_chassis_id,
	uint8_t *p_buf,
	uint16_t *p_pos)
{
...
static void pf_lldp_add_port_id_tlv(
	pf_lldp_port_id_t const *p_port_id,
	uint8_t *p_buf,
	uint16_t *p_pos)
{
...
static void pf_lldp_add_ieee_mac_phy(
	pf_lldp_link_status_t const *p_status,
	uint8_t *p_buf,
	uint16_t *p_pos)
{
...
static void pf_lldp_add_management(
	pf_lldp_management_address_t const *p_man_address,
	uint8_t *p_buf,
	uint16_t *p_pos)
{
...
static bool pf_lldp_management_address_is_ipv4(
	pf_lldp_management_address_t const *address)
{
...
{
	bool is_received;
	pf_port_t const *p_port_data = pf_port_get_state(net, loc_port_num);
...
{
	pnet_ethaddr_t const *device_mac_address =
...
size_t pf_lldp_construct_frame(pnet_t *net, int loc_port_num, uint8_t buf[])
{
	...
	char const *chassis_id_description = "<MAC address>";
#endif
...
static void pf_lldp_send(pnet_t *net, int loc_port_num)
{
	...
	{
		p_buffer->len = pf_lldp_construct_frame(net, loc_port_num, reinterpret_cast<uint8_t *>(p_buffer->payload));
...
bool pf_lldp_is_alias_matching(pnet_t *net, char const *alias)
{
...
void pf_lldp_store_peer_info(
	pnet_t *net,
	int loc_port_num,
	pf_lldp_peer_info_t const *new_info)
{
	pf_port_t *p_port_data = pf_port_get_state(net, loc_port_num);
...
void pf_lldp_update_peer(
	pnet_t *net,
	int loc_port_num,
	pf_lldp_peer_info_t const *lldp_peer_info)
{
	pf_port_t *p_port_data = pf_port_get_state(net, loc_port_num);
