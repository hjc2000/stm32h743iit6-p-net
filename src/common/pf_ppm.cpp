 ...
 */
char const *pf_ppm_state_to_string(pf_ppm_state_values_t state)
{
...
{
	uint8_t *p_payload = reinterpret_cast<uint8_t *>(((pnal_buf_t *)p_ppm->p_send_buffer)->payload);
	uint16_t u16;
...
int pf_ppm_create(pnet_t *net, pf_ar_t *p_ar, uint32_t crep)
{
	...
	pf_ppm_init_buf(
		p_ppm,
		reinterpret_cast<pnal_buf_t *>(p_ppm->p_send_buffer),
...
int pf_ppm_close_req(pnet_t *net, pf_ar_t *p_ar, uint32_t crep)
{
	...
	pnal_buf_free(reinterpret_cast<pnal_buf_t *>(p_ppm->p_send_buffer));
...
int pf_ppm_get_data_status(pf_ppm_t const *p_ppm, uint8_t *p_data_status)
{
	...
	uint16_t reduction_ratio)
{
	...
	uint32_t const transmission_interval_in_timebases =
...
void pf_ppm_show(pf_ppm_t const *p_ppm)
{
	printf("ppm:\n");
	printf(
		"   state                        = %s\n",
