...
static void pf_ppm_drv_sw_send(pnet_t *net, void *arg, uint32_t current_time)
{
	...
	if (p_arg->ppm.ci_running == true)
	{
		...
		/* Now send it */
		if (pf_eth_send_on_management_port(net, reinterpret_cast<pnal_buf_t *>(p_arg->ppm.p_send_buffer)) > 0)
		{
...
void pf_ppm_drv_sw_show(pf_ppm_t const *p_ppm)
{
...
{
	static pf_ppm_driver_t const drv = {
