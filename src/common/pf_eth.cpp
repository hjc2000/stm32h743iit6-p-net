 ...
 *          -1 on error
 */
static int pf_eth_init_netif(pnet_t *net,
							 ...
							 pf_netif_t *netif)
{
	...
	netif->handle = pnal_eth_init(netif->name,
								  eth_receive_type,
								  pnal_cfg,
								  pf_eth_recv,
								  (void *)net);
...
void pf_eth_frame_id_map_add(pnet_t *net,
							 uint16_t frame_id,
